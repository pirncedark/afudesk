...
pub fn host_kabul(kontrol: bool, dosya: bool) {
    host_komut(HostKomut::Kabul(Izinler { kontrol, pano: false, dosya }));
}
    ...
    rt().spawn(async move {
        while let Some(o) = alici.recv().await {
            let dto = match o {
                ...
                IzleyiciOlay::Dosya { ad, gonderilen, toplam, bitti, hata } => {
                    IzleyiciOlayi { tur: "dosya".into(), ad, gonderilen, toplam, bitti, metin: hata, ..Default::default() }
                }
            };
